...
def _artefacto(modo: str, si_sdr_emparejadas: list[float]) -> dict[str, Any]:
    return {
        "modo": modo,
        ...
        "reportes": [
            {
                ...
                "num_referencias": len(si_sdr_emparejadas),
...
def _escribir_artefacto(mediciones_dir: Path, modo: str, si_sdr_emparejadas: list[float]) -> None:
    mediciones_dir.mkdir(parents=True, exist_ok=True)
    (mediciones_dir / f"{modo}.json").write_text(json.dumps(_artefacto(modo, si_sdr_emparejadas)))
...
def test_solo_existe_submuestra_hito1_y_se_juzga_sin_que_el_otro_exista(
    ...
    existen (spec.md FR-008, US3 AS1)."""
    monkeypatch.chdir(tmp_path)
    _escribir_artefacto(tmp_path / "mediciones", "submuestra_hito1", [-3.0, -2.0])
...
def test_solo_existe_conjunto_completo_y_se_juzga_sin_que_el_otro_exista(
    ...
    que ambos modos comparan contra el mismo PRESUPUESTO_SI_SDR_DB."""
    monkeypatch.chdir(tmp_path)
