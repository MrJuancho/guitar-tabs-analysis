...
def _especificaciones_de_tema(
    draw: DrawFn,
) -> tuple[int, int, tuple[EspecificacionStem, ...]]:
    """Genera (frecuencia_muestreo, n_muestras_mezcla, stems de guitarra),
    cada stem con su propio `subtype` muestreado del dominio real."""
    ...
    return frecuencia_muestreo, n_muestras_mezcla, stems
...
def test_mezcla_y_guitarras_comparten_longitud_y_frecuencia_de_muestreo(
    ...
    especificacion: tuple[int, int, tuple[EspecificacionStem, ...]],
) -> None:
    ...
    frecuencia_muestreo, n_muestras_mezcla, stems = especificacion
