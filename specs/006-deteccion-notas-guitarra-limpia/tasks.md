...
- [ ] T002 (BLOQUEADO -- parcial) En `pyproject.toml`, agregar `basic-pitch[onnx]` (Apache-2.0, research.md #1), `mir_eval` (MIT, research.md #3/#4/#5) y `mirdata` (BSD-3-Clause, research.md #7) como dependencias directas -- **nunca** el extra `[tf]` de `basic-pitch` (research.md #2). Correr `uv lock` y confirmar en `/implement` que `onnxruntime` queda como único backend de inferencia disponible para `basic_pitch` (ningún paquete instalado arrastra `tensorflow` de forma transitiva).
  **Hallazgo de la sesión de `/speckit-implement` (T001-T009): `basic-pitch` NO se pudo agregar.** Verificado contra su `pyproject.toml` real (rama `main` de `github.com/spotify/basic-pitch`, idéntico a la versión `0.4.0` publicada en PyPI, la única disponible): su dependencia BASE -- fuera de cualquier extra, incondicional -- incluye `tensorflow>=2.4.1,<2.15.1; platform_system != 'Darwin' and python_version >= '3.11'`. Esa franja de `tensorflow` no publica ninguna rueda para `cp312` (confirmado contra PyPI: `tensorflow 2.15.0` solo trae `cp39`/`cp310`/`cp311`) -- instalar `basic-pitch` (con o sin `[onnx]`) es irresoluble en este proyecto (`requires-python = ">=3.12"`, Linux) sin importar qué extra se elija, no solo indeseable por research.md #2. `uv lock` con `basic-pitch[onnx]>=0.4` agregado falla con "No solution found". `mir_eval` y `mirdata` SÍ se agregaron y resuelven sin arrastrar `tensorflow` (T005-T009 solo los necesitan a ellos). Ver `pyproject.toml` (comentario junto a las dependencias) y `docs/ATRIBUCIONES.md` para el detalle completo. La instalación real de `basic-pitch` (User Story 2, T010 en adelante) queda pendiente de que se resuelva este bloqueo -- posibles salidas a evaluar en una sesión futura: esperar una versión de `basic-pitch` compatible con cp312, relajar `requires-python` para un entorno de inferencia aparte, o reconsiderar el modelo (research.md #1 tendría que reabrirse).
