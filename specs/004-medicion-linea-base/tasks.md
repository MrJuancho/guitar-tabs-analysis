...
- [ ] T014 Implementar `leer_manifiesto(directorio: Path) -> ManifiestoCorrida | None` y `escribir_manifiesto(directorio: Path, manifiesto: ManifiestoCorrida) -> None` en `orquestador.py` — lectura/escritura simple, **sin validación de firma todavía** (eso es User Story 2, T023). Depende de T006.
...
- [ ] T016 [P] [US1] Dos tests de integración sobre el mismo mecanismo (`ejecutar_corrida`, mismo archivo — no dos tareas, Principio X: verifican la misma unidad desde dos escenarios), en `tests/integration/test_orquestador_integracion.py` (nuevo):
