...
- [ ] T027 [P] Mutation testing acotado a la capa nueva de esta feature: `analytics.metrica_digitacion`, `digitacion.orquestador`, `digitacion.cli`, y la extensión de `ingestion.guitarset` (`leer_grabacion_con_posicion_real`, `NotaConPosicionReal` -- no todo el módulo, el resto ya se triagó en la Feature 006 T026). Triage, no conteo -- mismo criterio que Feature 006 T026: equivalentes documentados con su razón, aserciones débiles corregidas, nunca tests para escenarios que el código no puede producir. Verificar antes de correr que `pyproject.toml::[tool.mutmut].also_copy` no necesita ningún directorio nuevo (esta feature no agrega ningún script/recurso fuera de `src/`/`tests/`, a diferencia de la Feature 006 T026). Dónde se esperan sobrevivientes, por precedente de este proyecto: las ramas de motivo de exclusión de `InstanteExcluido` (T007a, tres motivos distinguibles -- mismo patrón que ya apareció en Features 002/003/004/006); el guard de `Δt`/`ε` de T012 (research.md #13, rama que datos reales nunca ejercitan porque `agrupar_en_instantes` garantiza `Δt > 0` por construcción -- candidato fuerte a equivalente documentado, no a test fabricado).
