...
- [X] T019 [P] [US3] Integration test: `SeparadorFalso` configurado para devolver un diccionario sin la clave `"guitar"` → `resultado.estimaciones == []`, sin excepción, en `tests/integration/test_separador_integracion.py` (US3 AS1, FR-009).
- [X] T020 [US3] Integration test: `SeparadorFalso` configurado para devolver `"guitar"` con energía nula (vector cero) → `resultado.estimaciones` tiene exactamente una `Estimacion`, con esas muestras exactas — no se omite ni se reclasifica en esta feature (esa clasificación es de Feature 002), en `tests/integration/test_separador_integracion.py` (US3 AS2, FR-010) (mismo archivo que T019, después de T019).

**Checkpoint**: ✅ hecho (T019-T020 en verde). Las tres user stories funcionan de forma independiente, sin ninguna tarea de implementación nueva en esta fase, como se anticipó.

---

## Phase 8: Polish & Cross-Cutting Concerns

- [X] T021 [P] Correr `just gauntlet` (ruff format --check + lint-imports + mypy --strict + tests/unit+integration+property con cobertura ≥90%, excluyendo `-m modelo_real` de la corrida rápida) y corregir cualquier hallazgo.
- [X] T022 [P] Correr `just mutation separacion.separador` (la lógica pura, no `demucs_separador` — mutar código que envuelve una librería externa pesada aporta poca señal) y resolver mutantes sobrevivientes, prestando atención particular al mensaje de `SeparacionFallidaError` (AGENTS.md, "Tests de excepciones": afirmar el mensaje completo).
- [X] T023 Ejecutar el ejemplo end-to-end de `quickstart.md` manualmente (la sección sin modelo real) y confirmar que la salida coincide con lo esperado; luego, si hay red disponible, correr también `uv run pytest -m modelo_real -v` y confirmar que el bloque de la Fase 6 aparece si el test se salta, y que las aserciones pasan si carga.
...
- **Hallazgo real durante T018 (no anticipado)**: el test de determinismo real dio rojo contra `DemucsSeparador` ya "completo" — `shifts=1` (valor por defecto de `demucs.api.Separator`) desplaza la entrada en el tiempo por una cantidad ALEATORIA en cada llamada (diferencias de hasta 0.35 en amplitud entre corridas sobre el mismo clip, verificado). Corregido con `shifts=0` explícito, ciclo rojo-primero respetado (el test falló contra el código real, no se escribió después del fix). Con `shifts=0`, dos corridas dan resultados bit a bit idénticos. Detalle en research.md #10.
- **Hallazgo operativo durante T022**: `just mutation separacion.separador` con la configuración original de `[tool.mutmut]` dio **22/32 mutantes en `timeout` y 0 en `killed`** — la corrida de tests de mutmut incluye `tests/` completo (`pytest_add_cli_args_test_selection = ["tests/"]`), y eso arrastra los tests `modelo_real` (T018): un mutante que produce una entrada patológica (p. ej. un tamaño de remuestreo desbordado) hace que la inferencia real con `htdemucs_6s` se cuelgue o tarde muchísimo, y mutmut lo reporta como `timeout`, no como una señal útil. Corregido agregando `-m "not modelo_real"` a `pytest_add_cli_args_test_selection` en `pyproject.toml` — afecta a mutation testing de todo el proyecto, no solo de esta feature, y es la corrección correcta: `modelo_real` nunca debe ser parte de ninguna compuerta automática repetida (research.md #7 ya lo establecía para `just gauntlet`; mutation testing es la misma categoría de compuerta).

  **Triage de T022** (con la corrección de arriba aplicada, sobre `separacion.separador` únicamente — `demucs_separador.py` queda fuera a propósito, mutar código que envuelve una librería externa pesada aporta poca señal):

  Primera corrida limpia: 154 mutantes, 32 sobrevivientes (10 en `SeparacionFallidaError.__init__`, 22 en `separar_guitarra`), 0 en timeout. Todos genuinos, ninguno equivalente:
  - **`SeparacionFallidaError.__init__` (10/10 sobrevivientes)**: el test existente (T009) solo comprobaba `"Track00001" in str(error)` y `"forma de audio no soportada" in str(error)` — exactamente el patrón débil que `AGENTS.md` ya documenta ("Tests de excepciones: afirma el mensaje completo"). Mutaciones al texto explicativo (mayúsculas, `XX...XX`, `tema_id=None`) no las notaba nadie. Corregido con `assert str(error) == "<mensaje completo exacto>"` y `assert error.tema_id == "Track00001"`.
  - **`separar_guitarra` (22/22 sobrevivientes), tres patrones reales**:
    1. **`detalle` de `TransformacionDeclarada` nunca se comprobaba** (11 mutantes: `detalle=None`, texto vacío, mayúsculas) — ninguno de los tests de T006-T008 miraba el contenido de `detalle`, solo `tipo`/`direccion`/`aplicada`. Corregido agregando la aserción exacta de `detalle` en T006, T007 y T008.
    2. **La rama "canal de entrada ya es 1, no hace falta duplicar" nunca se ejercitaba con aserción propia** (`aplicada=False` para `canales`/`entrada` cuando `audio_channels=1`, 2 mutantes) — T006 solo comprobaba la transformación de frecuencia. Corregido agregando la aserción de la transformación de canales de entrada en T006.
    3. **La rama "la salida del separador ya viene con un solo canal, no hace falta colapsar" nunca se ejercitaba en absoluto** (9 mutantes: `shape[0]`↔`shape[1]`, `reshape(None)`, `aplicada_salida`, `detalle_salida`) — todos los tests existentes que producen una estimación usan una salida de 2 canales (T008) o el eco plano por defecto de `SeparadorFalso` (1-D, nunca genuinamente bidimensional de forma `(1, N)`). Corregido con un test nuevo (`test_salida_de_un_solo_canal_no_se_reescala_ni_se_marca_aplicada`) que pasa una salida `(1, N)` explícita.
    4. Resto (`estimacion.identificador`, `estimacion.audio.frecuencia_muestreo`, `resultado.tema_id`, `resultado.modelo` mutados a `None`): ningún test comprobaba el `ResultadoSeparacionTema`/`Estimacion` completos, solo sus `transformaciones`. Corregido agregando esas aserciones a T008.

  Segunda corrida (tests corregidos): **153/154 mutantes matados (99,35%)**. Único sobreviviente aceptado, documentado con comentario (sin `# pragma: no mutate`, mismo criterio que `invalid="ignore"` en `metrica_separacion.py`): `reshape(-1)` → `reshape(-2)` es equivalente porque NumPy interpreta cualquier entero negativo pasado como forma completa como "aplanar", no solo `-1` (verificado empíricamente); un pragma en esa línea también suprimiría `reshape(-1)` → `reshape(None)`, que SÍ es observable (dejaría un array 2D `(1, N)` sin aplanar) y que el nuevo test del punto 3 sí mata.
