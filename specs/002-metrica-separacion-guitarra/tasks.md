...
- [X] T026 [P] Correr `just gauntlet` (ruff format --check + lint-imports + mypy --strict + tests/unit+integration+property con cobertura ≥90%) y corregir cualquier hallazgo.
- [X] T027 [P] Correr `just mutation analytics.metrica_separacion` sobre el módulo y resolver mutantes sobrevivientes (AGENTS.md; constitución Principio X) — prestar atención particular a los mensajes de las dos excepciones nuevas (`EstimacionIncompatibleError`, `ReferenciaEnergiaNulaError`): afirmar el mensaje completo, no solo que los identificadores aparezcan (AGENTS.md, "Tests de excepciones"). Ver "Triage T027" en Notes: un defecto real (NaN en la mediana) encontrado antes de poder correr mutation testing siquiera, más el triage de los 24 sobrevivientes iniciales. Cierre: 226/228 mutantes matados (99.1%), 2 sobrevivientes documentados sin pragma (comparten línea con una mutación real ya cubierta).
- [X] T028 Ejecutar el ejemplo end-to-end de `quickstart.md` manualmente y confirmar que la salida coincide con lo esperado (`SI-SDR (referencia == estimación): inf`, exclusión de `Track00002`, distribución `{1: 1}`). El ejemplo, tal como estaba escrito, no corría: `emparejar_tema([referencia], [estimacion_perfecta])` le faltaba el primer argumento posicional `tema_id` (la firma real es `emparejar_tema(tema_id, referencias, estimaciones)`) — quickstart.md nunca se había ejecutado de punta a punta desde que `tema_id` se agregó a la firma. Corregido a `emparejar_tema('Track00001', [referencia], [estimacion_perfecta])`; con ese fix, la salida coincide exactamente con lo documentado.
...
- **Triage T027 (2026-09-04)**: antes de que mutation testing pudiera siquiera correr, `mutmut`'s clean-run (ejecuta la suite completa sin mutar, como línea base) hizo que Hypothesis encontrara un contraejemplo real sobre `test_incluir_tema_completamente_sin_emparejar_nunca_mejora_la_mediana`: un tema con referencia idéntica bit a bit a su estimación (+∞ exacto) junto a un tema completamente sin emparejar (−∞) arman un pool de 2 elementos cuyos valores centrales son +∞ y −∞ — `statistics.median` los promedia, y esa suma es NaN en IEEE754 (`inf + -inf`), violando la premisa de FR-007/FR-008 de que la mediana aquí es un estadístico de orden puro. Esto NO es un mutante — es un defecto real y reproducible (confirmado con un script determinista, sin Hypothesis de por medio) en código ya mergeado. Corregido con ciclo test-rojo-primero: test de regresión (`test_pool_par_con_inf_y_menos_inf_como_valores_centrales_no_produce_nan`), confirmado en rojo, y una función `_mediana_orden()` que reemplaza `statistics.median()` — coincide con ella en todo pool donde el promedio de los dos valores centrales está bien definido, y resuelve al valor MÁS BAJO quirúrgicamente solo en el caso ambiguo (política pesimista, FR-008/Principio V: nunca ocultar un resultado malo).

  Con la línea base ya sana, `just mutation analytics.metrica_separacion` corrió 246 mutantes, 24 sobrevivientes. Triage:

  **Brechas reales, corregidas (aserciones débiles / cobertura faltante, código de producción ya correcto)**:
  - `si_sdr`: el `motivo=` de `EstimacionIncompatibleError` (ambos casos: distinta longitud, distinta frecuencia) mutado a `None`/mayúsculas sobrevivía — los dos tests correspondientes solo comprobaban los identificadores, nunca `.motivo` ni `str(error)` completo (mismo patrón de AGENTS.md "Tests de excepciones" ya corregido para los constructores, pero no para estos dos usos). Reforzados con el mensaje completo.
  - `si_sdr`: `10.0 * np.log10(...)` mutado a `11.0 * ...` sobrevivía — ningún test comparaba un SI-SDR finito contra un valor numérico conocido (los demás solo verifican ±∞ exactos o invarianza a la ganancia, insensibles a la constante). Nuevo test con un caso analítico exacto (`s=[10,-10]`, error ortogonal `[1,1]` → 20.0 dB exacto).
  - `emparejar_tema`: `identificador_referencia=None` en las DOS ramas que producen `motivo="sin_estimacion_disponible"` sobrevivía — exactamente lo anticipado antes de correr mutation testing: los tests de esas ramas comprobaban el conteo y el motivo, nunca A QUÉ referencia pertenecía cada entrada de `sin_pareja`. Reforzados ambos tests con los identificadores explícitos. (Los otros dos motivos, `energia_nula` y `estimacion_silenciosa`, ya tenían esa aserción desde que se escribieron — el hueco era específico de `sin_estimacion_disponible`.)
  - `emparejar_tema`: saturar la matriz de costos a una constante (`-_SENTINEL_COSTO` roto a `+_SENTINEL_COSTO`, colapsando `np.clip` a un solo valor) sobrevivía — todos los tests existentes construyen referencia/estimación en el mismo orden que su mejor pareja, así que una asignación ingenua por índice coincide con la óptima real y no la distingue. Nuevo test con parejas cruzadas (la mejor pareja de cada referencia está en el índice contrario al suyo).
  - `agregar_conjunto`: pasar `None` en vez de `entrada.tema_id` a `emparejar_tema` sobrevivía — ningún test comprobaba que `reporte.tema_id` correspondiera de vuelta a su `EntradaConjunto` cuando hay más de un tema evaluado. Aserción agregada al test existente de 3 temas.
  - `_mediana_orden` (la función nueva de este mismo hallazgo): mutar la condición par/impar o el índice del valor central sobrevivía porque el único pool impar ya probado tenía tamaño 1, donde las tres mutaciones coinciden con el comportamiento correcto por accidente aritmético. Nuevo test con un pool impar de 5 valores distintos.

  **Equivalentes confirmados, documentados con `# pragma: no mutate` y comentario (no generalidad pendiente, cada uno verificado, no solo razonado)**:
  - `.astype(np.float64)` vs `.astype(None)` (tres ocurrencias: `si_sdr` ×2, `_tiene_energia_nula`) — NumPy resuelve `dtype=None` al tipo flotante por defecto, `float64`, en cualquier plataforma soportada (confirmado empíricamente); mismo patrón que `unit="D"`/`"d"` de pandas ya documentado en AGENTS.md.
  - `float("-inf")` vs `float("-INF")` (`si_sdr`, `agregar_conjunto`) — `float()` parsea inf/nan sin distinguir mayúsculas; mismo patrón que `.encode("utf-8")`/`"UTF-8"` de AGENTS.md.
  - `zip(..., strict=True)` vs `strict=False`/`None`/omitido (`emparejar_tema`) — `linear_sum_assignment` garantiza `len(filas) == len(columnas)` por contrato (asignación biyectiva); no hay ningún input construible donde los modos de `zip` difieran.

  **Equivalente confirmado, documentado SOLO con comentario (sin pragma, porque comparte línea con una mutación real ya cubierta)**:
  - `invalid="ignore"` vs `None`/omitido dentro de `np.errstate(divide="ignore", invalid="ignore")` (`si_sdr`) — el warning "invalid" (0/0 → NaN) que ignora nunca puede dispararse en ese punto (`energia_referencia != 0` y `energia_estimacion != 0` ya están garantizados, y si `s_target` fuera cero, `e_noise` sería `shat` completo, de energía no nula). No se marca con pragma porque la MISMA línea también mutaría `divide="ignore"`, que sí tiene cobertura real (`test_si_sdr_referencia_como_su_propia_estimacion_da_infinito_exacto` usa `filterwarnings("error")` y fallaría si el warning de división dejara de ignorarse) — mismo criterio ya usado en `slakh2100.py` para `always_2d=False`. Quedan como los 2 únicos sobrevivientes finales, aceptados.

  **Cierre**: 226/228 mutantes matados (99.1%) contando solo lo que mutmut sigue generando tras los pragmas; el conteo original de 246 incluía las líneas ahora excluidas.
